    ...
    pickers.new(_, {
      ...
      finder = finders.new_table {
        ...
        entry_maker = function(entry)
          return {
            value = entry,
            display = entry.description .. "  [" .. (entry.tags[1] or "") .. "]",
...
local tasks = task_picker("SM Tasks", "list", {
  ...
  { key = "b",    fn = function(t)
    vim.schedule(function()
      vim.ui.input({ prompt = "Bullet type: " }, function(bt)
...
local backlog = task_picker("SM Backlog", "backlog", {
...
local menu = function()
  action_picker("SM Menu", {
    menu_entry("  Tasks (daily)", tasks),
    menu_entry("  Backlog", backlog),
